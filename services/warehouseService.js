...
M_NAME_OUTTB AS
(
SELECT M_NAME, SUM(OUT_SQM) AS OUT_SQM FROM OUTTB GROUP BY M_NAME
),
M_NAME_OUT_PHANLOAI AS
(
SELECT M_NAME, CASE WHEN OUT_SQM > 5000 THEN 'A' WHEN OUT_SQM <=5000 AND OUT_SQM >500 THEN 'B' ELSE 'C' END AS PHANLOAI FROM M_NAME_OUTTB
),
OUTTB2 AS
(
SELECT M090.M_CODE, M090.M_NAME, isnull(M_NAME_OUT_PHANLOAI.PHANLOAI,'C') AS PHANLOAI FROM M090 LEFT JOIN M_NAME_OUT_PHANLOAI ON M090.M_NAME = M_NAME_OUT_PHANLOAI.M_NAME
),
TONPHANLOAI AS
(
...
LEFT JOIN M090 ON M090.M_CODE = I222.M_CODE AND M090.CTR_CD = I222.CTR_CD
  ...
  SELECT PHANLOAI, SUM(IN_CFM_QTY*WIDTH_CD*1.0/1000) AS IN_SQM  FROM INTB GROUP BY PHANLOAI ORDER BY PHANLOAI ASC
  `;
  //console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  //console.log(checkkq);
...
FROM OUTTB3 
GROUP BY OUTTB3.PHANLOAI ORDER BY OUTTB3.PHANLOAI ASC
  `;
  //console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  //console.log(checkkq);
...
TON_TOTALTB AS
(
SELECT TON_I222.M_CODE, TON_I222.M_LOT_NO, (TON_I222.ROLL_QTY* TON_I222.IN_CFM_QTY) AS IN_CFM_QTY, TON_I222.GAP, CASE WHEN TON_I222.GAP < ${DATA.MOC1} THEN 'A' WHEN TON_I222.GAP >=${DATA.MOC1} AND TON_I222.GAP < ${DATA.MOC2} THEN 'B' ELSE 'C' END AS PHANLOAI 
FROM TON_I222
UNION ALL
SELECT TON_RETURN.M_CODE, TON_RETURN.M_LOT_NO, (TON_RETURN.ROLL_QTY * TON_RETURN.RETURN_QTY) AS IN_CFM_QTY, TON_RETURN.GAP,CASE WHEN TON_RETURN.GAP < ${DATA.MOC1} THEN 'A' WHEN TON_RETURN.GAP >=${DATA.MOC1} AND TON_RETURN.GAP < ${DATA.MOC2} THEN 'B' ELSE 'C' END AS PHANLOAI   FROM TON_RETURN
UNION ALL
SELECT TON_HOLDING.M_CODE, TON_HOLDING.M_LOT_NO, (TON_HOLDING.HOLDING_ROLL_QTY * TON_HOLDING.HOLDING_QTY) AS IN_CFM_QTY, TON_HOLDING.GAP,CASE WHEN TON_HOLDING.GAP < ${DATA.MOC1} THEN 'A' WHEN TON_HOLDING.GAP >=${DATA.MOC1} AND TON_HOLDING.GAP < ${DATA.MOC2} THEN 'B' ELSE 'C' END AS PHANLOAI   FROM TON_HOLDING
),
TON_TOTALTB2 AS
(
