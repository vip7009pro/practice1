...
exports.get_table_comments = async (req, res, DATA) => {
  try {
    const page = Math.max(1, Number(DATA?.page || 1));
    const pageSize = Math.min(500, Math.max(1, Number(DATA?.pageSize || 200)));
    const offset = (page - 1) * pageSize;
    ...
    if (schema) where.push('s.name LIKE @schema');
    if (table) where.push('t.name LIKE @table');
    if (search) where.push('(t.name LIKE @search OR CAST(ep.value AS NVARCHAR(4000)) LIKE @search)');
    const whereSql = where.length ? `AND ${where.join(' AND ')}` : '';
    ...
    const items = (listRes.recordset || []).map((r) => ({
      schema: String(r.schema_name),
      table: String(r.table_name),
      description: r.table_description ? String(r.table_description) : '',
    }));
...
exports.update_table_comment = async (req, res, DATA) => {
  try {
    ...
    if (!table) {
      return res.send({ tk_status: 'NG', message: 'Missing table' });
    }
    ...
    await pool.request().input('schema_name', schema).input('table_name', table).input('desc', description).query(sql);
    return res.send({ tk_status: 'OK', data: { schema, table, description } });
  } catch (e) {
    return res.send({ tk_status: 'NG', message: e?.message || String(e) });
  }
};

exports.bulk_upsert_comments = async (req, res, DATA) => {
  try {
    const items = Array.isArray(DATA?.items) ? DATA.items : [];
    if (items.length === 0) return res.send({ tk_status: 'NG', message: 'No items' });
    ...
    for (let i = 0; i < items.length; i++) {
      const raw = items[i] || {};
      const level = String(raw.level || raw.type || '').trim().toUpperCase();
      const schema = String(raw.schema || 'dbo').trim() || 'dbo';
      const table = String(raw.table || '').trim();
      const column = String(raw.column || '').trim();
      const description = raw.description === null || raw.description === undefined ? '' : String(raw.description);

      if (!table || (level === 'COLUMN' && !column) || (level !== 'COLUMN' && level !== 'TABLE')) {
        ngCount++;
        results.push({ index: i, ok: false, level, schema, table, column, error: 'Invalid row (need level TABLE/COLUMN and table/column)' });
        continue;
      }
      ...
      try {
        ...
        okCount++;
        results.push({ index: i, ok: true, level, schema, table, column });
      } catch (e) {
        ngCount++;
        results.push({ index: i, ok: false, level, schema, table, column, error: e?.message || String(e) });
      }
    }

    const ms = Date.now() - started;
    return res.send({ tk_status: 'OK', data: { ok: okCount, ng: ngCount, total: items.length, ms, results } });
  } catch (e) {
    return res.send({ tk_status: 'NG', message: e?.message || String(e) });
  }
};
