...
exports.inspect_daily_ppm_oqc = async (req, res, DATA) => {
  let checkkq = "OK";
  let condition = ` WHERE INSPECT_START_TIME BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE} 23:59:59' AND ZTBINSPECTNGTB_OQC.INSPECT_TOTAL_QTY <>0`;
  if (DATA.FACTORY !== "ALL") {
    condition += ` AND ZTBINSPECTNGTB_OQC.FACTORY='${DATA.FACTORY}'`;
  }
  if (DATA.codeArray.length === 1) {
    condition += ` AND ZTBINSPECTNGTB_OQC.G_CODE='${DATA.codeArray[0]}'`;
  } else if (DATA.codeArray.length > 1) {
    let codeString = ``;
    let codeArStr = DATA.codeArray.map((ele, index) => `'${ele}'`).join(",");
    condition += ` AND ZTBINSPECTNGTB_OQC.G_CODE IN (${codeArStr})`;
  }
  if (DATA.CUST_NAME_KD !== "") {
    //join customer name from string splitted by  comma  example: SEV, SEVT, SAMSUNG NETWORK to 'SEV','SEVT','SAMSUNG NETWORK'
    let custNameAr = DATA.CUST_NAME_KD.split(",")
...
SELECT CAST(INSPECT_START_TIME as date) AS INSPECT_DATE, SUM(INSPECT_TOTAL_QTY) AS INSPECT_TOTAL_QTY, SUM(ERR1+ ERR2 + ERR3 + ERR32+ERR4+ ERR5+ ERR6+ ERR7 +ERR8 +ERR9 +ERR10+ERR11) AS MATERIAL_NG, SUM(ERR12+ERR13+ERR14+ERR15+ERR16+ERR17+ERR18+ERR19+ERR20+ERR21+ERR22+ERR23+ERR24+ERR25+ERR26+ERR27+ERR28+ERR29+ERR30+ERR31) AS PROCESS_NG, SUM(ERR1+ ERR2 + ERR3 + ERR32+ERR4+ERR5+ERR6+ERR7+ERR8+ERR9+ERR10+ERR11+ERR12+ERR13+ERR14+ERR15+ERR16+ERR17+ERR18+ERR19+ERR20+ERR21+ERR22+ERR23+ERR24+ERR25+ERR26+ERR27+ERR28+ERR29+ERR30+ERR31) AS TOTAL_NG, CAST(SUM(ERR1+ERR2+ERR3+ERR32+ERR4+ERR5+ERR6+ERR7+ERR8+ERR9+ERR10+ERR11+ERR12+ERR13+ERR14+ERR15+ERR16+ERR17+ERR18+ERR19+ERR20+ERR21+ERR22+ERR23+ERR24+ERR25+ERR26+ERR27+ERR28+ERR29+ERR30+ERR31) as float)/CAST(SUM(INSPECT_TOTAL_QTY) as float)*1000000 AS TOTAL_PPM, CAST(SUM(ERR1+ ERR2 + ERR3 + ERR32+ERR4+ ERR5+ ERR6+ ERR7 +ERR8 +ERR9 +ERR10+ERR11) as float)/CAST(SUM(INSPECT_TOTAL_QTY) as float)*1000000 AS MATERIAL_PPM, CAST(SUM(ERR12+ERR13+ERR14+ERR15+ERR16+ERR17+ERR18+ERR19+ERR20+ERR21+ERR22+ERR23+ERR24+ERR25+ERR26+ERR27+ERR28+ERR29+ERR30+ERR31) as float)/CAST(SUM(INSPECT_TOTAL_QTY) as float)*1000000 AS PROCESS_PPM  FROM 
  ZTBINSPECTNGTB_OQC 
  LEFT JOIN P400 ON P400.PROD_REQUEST_NO = ZTBINSPECTNGTB_OQC.PROD_REQUEST_NO AND P400.CTR_CD = ZTBINSPECTNGTB_OQC.CTR_CD
  LEFT JOIN M110 ON M110.CUST_CD = P400.CUST_CD AND M110.CTR_CD = P400.CTR_CD
           ${condition} AND ZTBINSPECTNGTB_OQC.CTR_CD='${DATA.CTR_CD}'
            GROUP BY CAST(INSPECT_START_TIME as date)            
)
...
exports.inspect_yearly_ppm_oqc = async (req, res, DATA) => {
  ...
  if (DATA.codeArray.length === 1) {
    ...
    let codeArStr = DATA.codeArray.map((ele, index) => `'${ele}'`).join(",");
    condition += `
     AND ZTBINSPECTNGTB_OQC.G_CODE IN (${codeArStr})`;
  }
  if (DATA.CUST_NAME_KD !== "") {
    //join customer name from string splitted by  comma  example: SEV, SEVT, SAMSUNG NETWORK to 'SEV','SEVT','SAMSUNG NETWORK'
    let custNameAr = DATA.CUST_NAME_KD.split(",")
...
exports.loadIQC1table = async (req, res, DATA) => {
  ...
  if (DATA.SHOW_ALL === false) {
    condition += ` AND (IQCTB.TOTAL_RESULT IN (''OK'', ''PD'') OR (IQCTB.TOTAL_RESULT = ''NG'' AND IQCTB.NCR_COUNTERMEASURE = ''Y'')) `;
  }
  let setpdQuery = `
...
ZTB_MATERIAL_TB2 AS
(
SELECT DISTINCT CUST_CD, M_NAME, CTR_CD, M_THICKNESS, KEO_KEO, BOC_TACH, DIEN_TRO, TINH_DIEN, FT_IR, TACK FROM ZTB_MATERIAL_TB WHERE USE_YN = ''Y''
),
IQCTB AS
(
SELECT  IQC1_TABLE.IQC1_ID, IQC1_TABLE.NCR_ID, ZTB_IQC_NCRTB.DEFECT_IMAGE AS NCR_DEFECT_IMAGE, ZTB_IQC_NCRTB.COUNTERMEASURE AS NCR_COUNTERMEASURE, ZTB_IQC_NCRTB.COUNTERMEASURE_EXT AS NCR_COUNTERMEASURE_EXT, IQC1_TABLE.M_CODE, M090.M_NAME, M090.WIDTH_CD, IQC1_TABLE.M_LOT_NO, IQC1_TABLE.LOT_CMS, IQC1_TABLE.LOT_VENDOR, IQC1_TABLE.LOT_VENDOR AS LOT_VENDOR_IQC, I222.LOTNCC, IQC1_TABLE.CUST_CD, M110.CUST_NAME_KD, I222.EXP_DATE, INPUT_LENGTH_TB.TOTAL_CFM_QTY AS INPUT_LENGTH, INPUT_LENGTH_TB.TOTAL_ROLL, IQC1_TABLE.NQ_CHECK_ROLL,IQC1_TABLE.IQC_TEST_RESULT,IQC1_TABLE.DTC_RESULT, IQC1_TABLE.CHECKSHEET, IQC1_TABLE.DTC_ID, IQC1_TABLE.TEST_EMPL, IQC1_TABLE.TOTAL_RESULT,'+@str3 +'
, 
IQC1_TABLE.INS_DATE, IQC1_TABLE.INS_EMPL, IQC1_TABLE.UPD_DATE, IQC1_TABLE.UPD_EMPL, IQC1_TABLE.REMARK, ZTB_MATERIAL_TB2.M_THICKNESS, IQC1_TABLE.THICKNESS, IQC1_TABLE.M_WIDTH, ISNULL(ZTB_MATERIAL_TB2.KEO_KEO, ''Y'') AS KEO_KEO, ISNULL(ZTB_MATERIAL_TB2.BOC_TACH, ''Y'') AS BOC_TACH, ISNULL(ZTB_MATERIAL_TB2.DIEN_TRO, ''Y'') AS DIEN_TRO, ISNULL(ZTB_MATERIAL_TB2.TINH_DIEN, ''Y'') AS TINH_DIEN, ISNULL(ZTB_MATERIAL_TB2.FT_IR, ''Y'') AS FT_IR, ISNULL(ZTB_MATERIAL_TB2.TACK, ''Y'') AS TACK FROM IQC1_TABLE 
LEFT JOIN ZTB_IQC_NCRTB ON (IQC1_TABLE.NCR_ID = ZTB_IQC_NCRTB.NCR_ID AND IQC1_TABLE.CTR_CD = ZTB_IQC_NCRTB.CTR_CD)
LEFT JOIN 
...
exports.insertIQC1table = async (req, res, DATA) => {
  let EMPL_NO = req.payload_data["EMPL_NO"];
  let checkkq = "OK";
  // FE có thể không gửi LOT_VENDOR (dòng cũ chỉ có LOT_VENDOR_IQC) -> mặc định "" để không crash
  let LOT_VENDOR = (DATA.LOT_VENDOR ?? "").toString();
...
exports.update_iqc_ncr_id = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `UPDATE IQC1_TABLE SET NCR_ID=${DATA.NCR_ID ? `'${DATA.NCR_ID}'` : 'NULL'} WHERE CTR_CD='${DATA.CTR_CD}' AND IQC1_ID=${DATA.IQC1_ID}`;
  console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  res.send(checkkq);
};
...
ORDER BY 
  ...
  DEFECT_MANAGEMENT.UPD_EMPL, 
  QTR_YN,
  DEFECT_MANAGEMENT.PART_CODE_OTHERS
FROM 
  DEFECT_MANAGEMENT 
  LEFT JOIN (
    SELECT 
...
INSP_TABLE3 AS
(
SELECT 30 * 12* 60 AS FULL_TIME, isnull(ZTBATTENDANCETB2.WORK_HOUR,0) AS WORK_HOUR, isnull(ZTBATTENDANCETB2.WORK_MINUTE,0) AS  WORK_MINUTE, INSP_TABLE2.*, 
...
FROM INSP_TABLE2
LEFT JOIN ZTBATTENDANCETB2 ON INSP_TABLE2.EMPL_NO = ZTBATTENDANCETB2.EMPL_NO
)
SELECT INSP_TABLE3.*, 
CASE 
...
CASE 
WHEN COUNT_PS = 0 AND COUNT_TT <>  0 THEN (RATE_TT + (1-TRU_DIEM * 0.05) + TILE_CC)*1.0/3
WHEN COUNT_PS <> 0 AND COUNT_TT =  0 THEN (RATE_PS + (1-TRU_DIEM * 0.05) + TILE_CC)*1.0/3
WHEN COUNT_PS <> 0 AND COUNT_TT <>  0 THEN ((RATE_PS + RATE_TT)/2 +(1-TRU_DIEM * 0.05) + TILE_CC)*1.0/3
ELSE 0
END AS FINAL_KPI,
...
exports.updateMaterialTestItem = async (req, res, DATA) => {
  let checkkq = "OK";
  const allowedFields = ["KEO_KEO", "BOC_TACH", "DIEN_TRO", "TINH_DIEN", "FT_IR", "TACK"];
  if (!allowedFields.includes(DATA.FIELD_NAME)) {
    return res.send({ tk_status: "NG", message: "Invalid field name" });
  }
  let setpdQuery = `
    UPDATE ZTB_MATERIAL_TB 
    SET ${DATA.FIELD_NAME} = '${DATA.VAL}' 
    WHERE CTR_CD = '${DATA.CTR_CD}' AND M_NAME = '${DATA.M_NAME}'
  `;
  console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  res.send(checkkq);
