...
exports.loadMaterialByYCSX = async (req, res, DATA) => {
  ...
  WITH BANG_MRP AS
(
SELECT  P400.PROD_REQUEST_NO, P400.PROD_REQUEST_DATE, M110.CUST_CD, M110.CUST_NAME_KD, M100.G_CODE, M100.G_NAME_KD, ZTB_BOM2.M_CODE, M090.M_NAME, M090.WIDTH_CD, P400.PROD_REQUEST_QTY, M100.PD, M100.G_C AS CAVITY_COT, M100.G_C_R AS CAVITY_HANG, M100.G_C * M100.G_C_R AS CAVITY, CAST(( P400.PROD_REQUEST_QTY*1.0)*(M100.PD*1.0)/(M100.G_C *M100.G_C_R*1000)AS bigint) AS NEED_M_QTY, P400.MATERIAL_YN,ZTB_MATERIAL_TB.CMSPRICE AS ORIGIN_PRICE, isnull(M100.LOSS_SETTING1,0) AS LOSS_SETTING1, isnull(M100.LOSS_SETTING2,0) AS LOSS_SETTING2, isnull(M100.LOSS_SETTING3,0) AS LOSS_SETTING3, isnull(M100.LOSS_SETTING4,0) AS LOSS_SETTING4, isnull(M100.LOSS_SX1,0) AS LOSS_SX1, isnull(M100.LOSS_SX2,0) AS LOSS_SX2,isnull(M100.LOSS_SX3,0) AS LOSS_SX3, isnull(M100.LOSS_SX4,0) AS LOSS_SX4
    FROM P400 
