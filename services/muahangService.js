...
MSDS_TB.VER AS MSDS_VER,ZTB_MATERIAL_TB.TDS,
...
exports.loadMaterialByYCSX = async (req, res, DATA) => {
  ...
  WITH BANG_MRP AS
(
SELECT  P400.PROD_REQUEST_NO, P400.PROD_REQUEST_DATE, M110.CUST_CD, M110.CUST_NAME_KD, M100.G_CODE, M100.G_NAME_KD, ZTB_BOM2.M_CODE, M090.M_NAME, M090.WIDTH_CD, P400.PROD_REQUEST_QTY, M100.PD, M100.G_C AS CAVITY_COT, M100.G_C_R AS CAVITY_HANG, M100.G_C * M100.G_C_R AS CAVITY, CAST(( P400.PROD_REQUEST_QTY*1.0)*(M100.PD*1.0)/(M100.G_C *M100.G_C_R*1000)AS bigint) AS NEED_M_QTY, P400.MATERIAL_YN,ZTB_MATERIAL_TB.SSPRICE AS OPEN_PRICE, isnull(M100.LOSS_SETTING1,0) AS LOSS_SETTING1, isnull(M100.LOSS_SETTING2,0) AS LOSS_SETTING2, isnull(M100.LOSS_SETTING3,0) AS LOSS_SETTING3, isnull(M100.LOSS_SETTING4,0) AS LOSS_SETTING4, isnull(M100.LOSS_SX1,0) AS LOSS_SX1, isnull(M100.LOSS_SX2,0) AS LOSS_SX2,isnull(M100.LOSS_SX3,0) AS LOSS_SX3, isnull(M100.LOSS_SX4,0) AS LOSS_SX4, M1102.CUST_NAME_KD AS NCC
    FROM P400 
    ...
    LEFT JOIN M090 ON (ZTB_BOM2.M_CODE = M090.M_CODE AND ZTB_BOM2.CTR_CD = M090.CTR_CD)
	LEFT JOIN ZTB_MATERIAL_TB ON (M090.M_NAME = ZTB_MATERIAL_TB.M_NAME AND M090.CTR_CD = ZTB_MATERIAL_TB.CTR_CD AND ZTB_MATERIAL_TB.USE_YN ='Y') -- join vao bi double  do liệu trùng M_NAME chưa set USe_YN='N'
	LEFT JOIN M110 AS M1102 ON (M1102.CUST_CD = ZTB_MATERIAL_TB.CUST_CD AND M1102.CTR_CD = ZTB_MATERIAL_TB.CTR_CD)
   ${condition}
    AND P400.CTR_CD='${DATA.CTR_CD}'
    )
SELECT BANG_MRP.*,(LOSS_SETTING1 + LOSS_SETTING2 + LOSS_SETTING3 + LOSS_SETTING4) AS LOSS_SETTING,(LOSS_SX1+ LOSS_SX2+ LOSS_SX3+ LOSS_SX4) AS LOSS_SX, (LOSS_SETTING1 + LOSS_SETTING2 + LOSS_SETTING3 + LOSS_SETTING4) + BANG_MRP.NEED_M_QTY*(100+LOSS_SX1+ LOSS_SX2+ LOSS_SX3+ LOSS_SX4)/100 AS FINAL_QTY  FROM BANG_MRP ORDER BY NEED_M_QTY DESC
    `;
  //console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  //console.log(checkkq);
