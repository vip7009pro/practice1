...
exports.getBOMAMAZON = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `SELECT BOM_AMAZONE.AMZ_PROD_NAME, BOM_AMAZONE.AMZ_COUNTRY, BOM_AMAZONE.G_CODE, M100.G_NAME, DESIGN_AMAZONE.G_CODE_MAU,  M100_B.G_NAME AS TEN_MAU,BOM_AMAZONE.DOITUONG_NO, DESIGN_AMAZONE.DOITUONG_NAME, BOM_AMAZONE.GIATRI, BOM_AMAZONE.REMARK, BOM_AMAZONE.DOITUONG_NAME2,DESIGN_AMAZONE.PHANLOAI_DT FROM BOM_AMAZONE LEFT JOIN DESIGN_AMAZONE ON (BOM_AMAZONE.G_CODE_MAU= DESIGN_AMAZONE.G_CODE_MAU AND BOM_AMAZONE.DOITUONG_NO= DESIGN_AMAZONE.DOITUONG_NO AND BOM_AMAZONE.CTR_CD = DESIGN_AMAZONE.CTR_CD) LEFT JOIN M100 ON (M100.G_CODE = BOM_AMAZONE.G_CODE AND M100.CTR_CD = BOM_AMAZONE.CTR_CD)  LEFT JOIN  (SELECT * FROM M100) AS M100_B ON (M100_B.G_CODE = DESIGN_AMAZONE.G_CODE_MAU AND M100_B.CTR_CD = DESIGN_AMAZONE.CTR_CD) WHERE BOM_AMAZONE.CTR_CD='${DATA.CTR_CD}' AND BOM_AMAZONE.G_CODE='${DATA.G_CODE}'`;
  ////console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  res.send(checkkq);
}
...
exports.updateAmazonBOM = async (req, res, DATA) => {
  let checkkq = "OK";
  let DTN2 = (DATA.DOITUONG_NAME2 === undefined || DATA.DOITUONG_NAME2 === 'null' || DATA.DOITUONG_NAME2 === "") ? null : `'${DATA.DOITUONG_NAME2}'`;
  let setpdQuery = `UPDATE BOM_AMAZONE SET GIATRI='${DATA.GIATRI}', REMARK = '${DATA.REMARK}',  AMZ_COUNTRY='${DATA.AMZ_COUNTRY}', DOITUONG_NAME2=${DTN2} WHERE CTR_CD='${DATA.CTR_CD}' AND G_CODE='${DATA.G_CODE}' AND G_CODE_MAU='${DATA.G_CODE_MAU}' AND DOITUONG_NO=${DATA.DOITUONG_NO}`;
  console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  res.send(checkkq);
}
...
exports.loadSampleMonitorTable = async (req, res, DATA) => {
  ...
  let setpdQuery = `
    ...
    P400 ON P400.PROD_REQUEST_NO = ZTB_SAMPLE_MONITOR.PROD_REQUEST_NO AND P400.CTR_CD = ZTB_SAMPLE_MONITOR.CTR_CD 
	LEFT JOIN M100 ON ZTB_SAMPLE_MONITOR.G_CODE = M100.G_CODE AND ZTB_SAMPLE_MONITOR.CTR_CD = M100.CTR_CD 
...
exports.addMonitoringSample = async (req, res, DATA) => {
  ...
  let checkkq = "OK";
  let setpdQuery = `
    INSERT INTO ZTB_SAMPLE_MONITOR (CTR_CD, PROD_REQUEST_NO, G_NAME_KD, INS_EMPL, UPD_EMPL,INS_DATE, UPD_DATE, REQ_ID, G_CODE) VALUES ('${DATA.CTR_CD}','${DATA.PROD_REQUEST_NO}','${DATA.G_NAME_KD}','${EMPL_NO}','${EMPL_NO}',GETDATE(),GETDATE(),${DATA.REQ_ID},'${DATA.G_CODE}')
    `;
  console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  res.send(checkkq);
...
exports.addYCTK = async (req, res, DATA) => {
  let EMPL_NO = req.payload_data["EMPL_NO"];
  let checkkq = "OK";
  let setpdQuery = `INSERT INTO ZTB_YCTK_TB (CTR_CD, CUST_CD, G_CODE, DESIGN_REQUEST_DATE, ESTIMATED_PO, MATERIAL, Coating_KhongPhu, Coating_LamiBong, Coating_OpamMo, Coating_UVBong, Coating_UVMo, LabelWidth, LabelHeight, Tolerance, FaceOut, FaceIn, TopBottomSpacing, BetweenLabelSpacing, LinerSpacing, CornerRadius, AdhesiveRemoval, HasToothCut, DieCutType, LabelForm, LabelFormQty, RollCore, PrintYN, DecalType, ApproveType, SpecialRequirement, SpecialRequirementLevel, RND_EMPL, QC_EMPL, KD_EMPL, INS_DATE, INS_EMPL, UPD_DATE, UPD_EMPL, Co_Sx_Mau, ManualCloseStatus) VALUES ('${DATA.CTR_CD}', '${DATA.CUST_CD}', '${DATA.G_CODE}', '${DATA.DESIGN_REQUEST_DATE}', ${DATA.ESTIMATED_PO}, N'${DATA.MATERIAL}', ${DATA.Coating_KhongPhu? 1: 0}, ${DATA.Coating_LamiBong? 1: 0}, ${DATA.Coating_OpamMo? 1: 0}, ${DATA.Coating_UVBong? 1: 0}, ${DATA.Coating_UVMo? 1: 0}, ${DATA.LabelWidth}, ${DATA.LabelHeight}, ${DATA.Tolerance}, ${DATA.FaceOut}, ${DATA.FaceIn}, ${DATA.TopBottomSpacing}, ${DATA.BetweenLabelSpacing}, ${DATA.LinerSpacing}, ${DATA.CornerRadius}, ${DATA.AdhesiveRemoval? 1: 0}, ${DATA.HasToothCut? 1: 0}, N'${DATA.DieCutType}', N'${DATA.LabelForm}', ${DATA.LabelFormQty}, N'${DATA.RollCore}', N'${DATA.PrintYN}', N'${DATA.DecalType}', N'${DATA.ApproveType}', N'${DATA.SpecialRequirement}', ${DATA.SpecialRequirementLevel},'${EMPL_NO}', '${EMPL_NO}', '${EMPL_NO}', GETDATE(), '${EMPL_NO}', GETDATE(), '${EMPL_NO}', '${DATA.Co_Sx_Mau}', '${DATA.ManualCloseStatus}')`;
  console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
  //console.log(checkkq);
  res.send(checkkq);
...
SELECT ZTB_YCTK_TB.*, M100.G_NAME, M100.G_NAME_KD, M110.CUST_NAME_KD,CASE WHEN APPROVED_SAMPLE_TABLE.REQ_ID is null THEN 'P' ELSE 'Y' END AS SAMPLE_STATUS, 
CASE WHEN ZTB_YCTK_TB.Co_Sx_Mau =0 THEN 
	CASE WHEN ZTB_YCTK_TB.ManualCloseStatus = 1 THEN 'Y' ELSE 'P' END
ELSE CASE WHEN APPROVED_SAMPLE_TABLE.REQ_ID is null THEN 'P' ELSE 'Y' END
END AS FINAL_STATUS
FROM ZTB_YCTK_TB LEFT JOIN M100 ON M100.G_CODE = ZTB_YCTK_TB.G_CODE AND M100.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN M110 ON M110.CUST_CD = ZTB_YCTK_TB.CUST_CD AND M110.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN APPROVED_SAMPLE_TABLE ON APPROVED_SAMPLE_TABLE.CTR_CD = ZTB_YCTK_TB.CTR_CD AND APPROVED_SAMPLE_TABLE.REQ_ID = ZTB_YCTK_TB.REQ_ID 
...
FROM ZTB_YCTK_TB LEFT JOIN M100 ON M100.G_CODE = ZTB_YCTK_TB.G_CODE AND M100.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN M110 ON M110.CUST_CD = ZTB_YCTK_TB.CUST_CD AND M110.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN APPROVED_SAMPLE_TABLE ON APPROVED_SAMPLE_TABLE.CTR_CD = ZTB_YCTK_TB.CTR_CD AND APPROVED_SAMPLE_TABLE.REQ_ID = ZTB_YCTK_TB.REQ_ID 
  ...
  SELECT YEAR(DESIGN_REQUEST_DATE) AS REQ_YEAR, DATEPART(ISO_WEEK,DESIGN_REQUEST_DATE) AS REQ_WEEK, CONCAT(YEAR(DESIGN_REQUEST_DATE),'_', DATEPART(ISO_WEEK,DESIGN_REQUEST_DATE)) AS REQ_YW, SUM(CASE WHEN FINAL_STATUS = 'Y' THEN 1 ELSE 0 END) AS COMPLETED, SUM(CASE WHEN FINAL_STATUS = 'P' THEN 1 ELSE 0 END) AS PENDING, COUNT(FINAL_STATUS) AS TOTAL  
  FROM YCTKTB 
...
FROM ZTB_YCTK_TB LEFT JOIN M100 ON M100.G_CODE = ZTB_YCTK_TB.G_CODE AND M100.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN M110 ON M110.CUST_CD = ZTB_YCTK_TB.CUST_CD AND M110.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN APPROVED_SAMPLE_TABLE ON APPROVED_SAMPLE_TABLE.CTR_CD = ZTB_YCTK_TB.CTR_CD AND APPROVED_SAMPLE_TABLE.REQ_ID = ZTB_YCTK_TB.REQ_ID 
  ...
  SELECT YEAR(DESIGN_REQUEST_DATE) AS REQ_YEAR, MONTH(DESIGN_REQUEST_DATE) AS REQ_MONTH, CONCAT(YEAR(DESIGN_REQUEST_DATE),'_', MONTH(DESIGN_REQUEST_DATE)) AS REQ_YM, SUM(CASE WHEN FINAL_STATUS = 'Y' THEN 1 ELSE 0 END) AS COMPLETED, SUM(CASE WHEN FINAL_STATUS = 'P' THEN 1 ELSE 0 END) AS PENDING, COUNT(FINAL_STATUS) AS TOTAL  
  FROM YCTKTB 
...
FROM ZTB_YCTK_TB LEFT JOIN M100 ON M100.G_CODE = ZTB_YCTK_TB.G_CODE AND M100.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN M110 ON M110.CUST_CD = ZTB_YCTK_TB.CUST_CD AND M110.CTR_CD = ZTB_YCTK_TB.CTR_CD  LEFT JOIN APPROVED_SAMPLE_TABLE ON APPROVED_SAMPLE_TABLE.CTR_CD = ZTB_YCTK_TB.CTR_CD AND APPROVED_SAMPLE_TABLE.REQ_ID = ZTB_YCTK_TB.REQ_ID 
  ...
  SELECT YEAR(DESIGN_REQUEST_DATE) AS REQ_YEAR, SUM(CASE WHEN FINAL_STATUS = 'Y' THEN 1 ELSE 0 END) AS COMPLETED, SUM(CASE WHEN FINAL_STATUS = 'P' THEN 1 ELSE 0 END) AS PENDING, COUNT(FINAL_STATUS) AS TOTAL  
  FROM YCTKTB 
...
exports.tilefilmbandaily = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `
  -- ti le phim ban daily
  WITH
  FILM_M100 AS 
  (
  SELECT G_CODE, (G_SG_L +(G_CG + G_WIDTH) * (G_C - 1) +G_WIDTH +G_SG_R) + 20 AS F_WIDTH_LT,((G_LENGTH + G_LG) * G_C_R) + 20 AS F_LENGTH_LT, PROD_PRINT_TIMES FROM M100 
  ),
  FILM_M100_SQM AS
  (
  SELECT G_CODE,F_WIDTH_LT * F_LENGTH_LT * PROD_PRINT_TIMES *1.0/1000000 AS FILM_SQM, PROD_PRINT_TIMES,F_WIDTH_LT,F_LENGTH_LT  FROM FILM_M100
  ),
OUT_FILM_BACKDATA AS 
(
SELECT YEAR(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_YEAR, DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_WEEK, MONTH(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_MONTH, CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YW,CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',MONTH(ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YM, ZTB_QLSXPLAN.PLAN_DATE, ZTB_QLSXPLAN.PLAN_ID, OUT_KNIFE_FILM.KNIFE_FILM_NO, ZTB_QLSXPLAN.G_CODE , F_LENGTH, F_WIDTH, F_LENGTH_LT, F_WIDTH_LT, 
CASE WHEN OUT_KNIFE_FILM.F_NEW='Y' THEN  QTY_KNIFE_FILM ELSE 0 END AS QTY_KNIFE_FILM
, (F_LENGTH*F_WIDTH*(CASE WHEN OUT_KNIFE_FILM.F_NEW='Y' THEN  QTY_KNIFE_FILM ELSE 0 END))*1.0/1000000 AS OUT_SQM, FILM_M100_SQM.FILM_SQM,FILM_M100_SQM.PROD_PRINT_TIMES, OUT_KNIFE_FILM.F_NEW
FROM OUT_KNIFE_FILM 
LEFT JOIN ZTB_QLSXPLAN ON(ZTB_QLSXPLAN.PLAN_ID = OUT_KNIFE_FILM.PLAN_ID) 
LEFT JOIN FILM_M100_SQM ON (FILM_M100_SQM.G_CODE =ZTB_QLSXPLAN.G_CODE)
WHERE LOAIBANGIAO_PDP ='F' AND ZTB_QLSXPLAN.G_CODE is not null
)
  SELECT PLAN_DATE, SUM(QTY_KNIFE_FILM) AS FILM_QTY_TT, SUM(PROD_PRINT_TIMES) AS FILM_QTY_LT, SUM(OUT_SQM)*10000 AS SQM_TT, SUM(FILM_SQM)*10000 AS SQM_LT FROM OUT_FILM_BACKDATA 
  WHERE PLAN_DATE BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE}'
...
exports.tilefilmbanweekly = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `
-- ti le phim ban weekly
WITH
FILM_M100 AS 
(
  SELECT G_CODE, (G_SG_L +(G_CG + G_WIDTH) * (G_C - 1) +G_WIDTH +G_SG_R) + 20 AS F_WIDTH_LT,((G_LENGTH + G_LG) * G_C_R) + 20 AS F_LENGTH_LT, PROD_PRINT_TIMES FROM M100 
...
SELECT PLAN_YEAR,PLAN_WEEK,PLAN_YW, SUM(QTY_KNIFE_FILM) AS FILM_QTY_TT, SUM(PROD_PRINT_TIMES) AS FILM_QTY_LT, SUM(OUT_SQM)*10000 AS SQM_TT, SUM(FILM_SQM)*10000 AS SQM_LT FROM OUT_FILM_BACKDATA 
WHERE PLAN_DATE BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE}'
GROUP BY PLAN_YEAR,PLAN_WEEK,PLAN_YW ORDER BY PLAN_YEAR DESC, PLAN_WEEK DESC, PLAN_YW DESC
...
exports.tilefilmbanmonthly = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `
-- ti le phim ban monthly
WITH
FILM_M100 AS 
(
SELECT G_CODE, (G_SG_L +(G_CG + G_WIDTH) * (G_C - 1) +G_WIDTH +G_SG_R) + 20 AS F_WIDTH_LT,((G_LENGTH + G_LG) * G_C_R) + 20 AS F_LENGTH_LT, PROD_PRINT_TIMES FROM M100 
...
SELECT PLAN_YEAR,PLAN_MONTH,PLAN_YM, SUM(QTY_KNIFE_FILM) AS FILM_QTY_TT, SUM(PROD_PRINT_TIMES) AS FILM_QTY_LT, SUM(OUT_SQM)*10000 AS SQM_TT, SUM(FILM_SQM)*10000 AS SQM_LT FROM OUT_FILM_BACKDATA 
WHERE PLAN_DATE BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE}'
GROUP BY PLAN_YEAR,PLAN_MONTH,PLAN_YM ORDER BY PLAN_YEAR DESC, PLAN_MONTH DESC, PLAN_YM DESC
...
exports.tilefilmbanyearly = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `
-- ti le phim ban yearly
WITH
FILM_M100 AS 
(
SELECT G_CODE, (G_SG_L +(G_CG + G_WIDTH) * (G_C - 1) +G_WIDTH +G_SG_R) + 20 AS F_WIDTH_LT,((G_LENGTH + G_LG) * G_C_R) + 20 AS F_LENGTH_LT, PROD_PRINT_TIMES FROM M100 
...
SELECT PLAN_YEAR, SUM(QTY_KNIFE_FILM) AS FILM_QTY_TT, SUM(PROD_PRINT_TIMES) AS FILM_QTY_LT, SUM(OUT_SQM)*10000 AS SQM_TT, SUM(FILM_SQM)*10000 AS SQM_LT FROM OUT_FILM_BACKDATA 
WHERE PLAN_DATE BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE}'
GROUP BY PLAN_YEAR ORDER BY PLAN_YEAR DESC
`;
  //${moment().format('YYYY-MM-DD')}
  ////console.log(setpdQuery);
  checkkq = await queryDB(setpdQuery);
...
exports.tilefilmbanbackdata = async (req, res, DATA) => {
  let checkkq = "OK";
  let setpdQuery = `WITH
FILM_M100 AS 
(
...
FILM_M100_SQM AS
(
SELECT G_CODE,F_WIDTH_LT * F_LENGTH_LT * PROD_PRINT_TIMES *1.0/100 AS FILM_SQM, PROD_PRINT_TIMES,F_WIDTH_LT,F_LENGTH_LT  FROM FILM_M100
),
OUT_FILM_BACKDATA AS 
(
SELECT YEAR(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_YEAR, DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_WEEK, MONTH(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_MONTH, CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YW,CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',MONTH(ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YM, ZTB_QLSXPLAN.PLAN_DATE, ZTB_QLSXPLAN.PLAN_ID, OUT_KNIFE_FILM.KNIFE_FILM_NO, ZTB_QLSXPLAN.G_CODE , F_LENGTH, F_WIDTH, F_LENGTH_LT, F_WIDTH_LT, 
CASE WHEN OUT_KNIFE_FILM.F_NEW='Y' THEN  QTY_KNIFE_FILM ELSE 0 END AS QTY_KNIFE_FILM
, (F_LENGTH*F_WIDTH*(CASE WHEN OUT_KNIFE_FILM.F_NEW='Y' THEN  QTY_KNIFE_FILM ELSE 0 END))*1.0/100 AS OUT_SQM, FILM_M100_SQM.FILM_SQM,FILM_M100_SQM.PROD_PRINT_TIMES, OUT_KNIFE_FILM.F_NEW
FROM OUT_KNIFE_FILM 
LEFT JOIN ZTB_QLSXPLAN ON(ZTB_QLSXPLAN.PLAN_ID = OUT_KNIFE_FILM.PLAN_ID) 
LEFT JOIN FILM_M100_SQM ON (FILM_M100_SQM.G_CODE =ZTB_QLSXPLAN.G_CODE)
WHERE LOAIBANGIAO_PDP ='F' AND ZTB_QLSXPLAN.G_CODE is not null
)
SELECT * FROM OUT_FILM_BACKDATA
WHERE PLAN_DATE BETWEEN '${DATA.FROM_DATE}' AND '${DATA.TO_DATE}'
`;
  //${moment().format('YYYY-MM-DD')}
  //console.log(setpdQuery);
