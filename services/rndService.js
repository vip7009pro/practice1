...
exports.tilefilmbandaily = async (req, res, DATA) => {
  ...
  FILM_M100 AS 
  (
  SELECT G_CODE, (G_SG_L +(G_CG + G_WIDTH) * (G_C - 1) +G_WIDTH +G_SG_R) AS F_WIDTH_LT,((G_LENGTH + G_LG) * G_C_R) AS F_LENGTH_LT, PROD_PRINT_TIMES FROM M100 
  ),
  FILM_M100_SQM AS
  (
  ...
  , (F_LENGTH*F_WIDTH*QTY_KNIFE_FILM)*1.0/1000000 AS OUT_SQM, FILM_M100_SQM.FILM_SQM,FILM_M100_SQM.PROD_PRINT_TIMES, OUT_KNIFE_FILM.F_NEW
  FROM OUT_KNIFE_FILM 
...
SELECT YEAR(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_YEAR, DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_WEEK, MONTH(ZTB_QLSXPLAN.PLAN_DATE) AS PLAN_MONTH, CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',DATEPART(ISO_WEEK, ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YW,CONCAT(YEAR(ZTB_QLSXPLAN.PLAN_DATE),'_',MONTH(ZTB_QLSXPLAN.PLAN_DATE)) AS PLAN_YM, ZTB_QLSXPLAN.PLAN_DATE, ZTB_QLSXPLAN.PLAN_ID, OUT_KNIFE_FILM.KNIFE_FILM_NO, ZTB_QLSXPLAN.G_CODE , F_LENGTH, F_WIDTH, F_LENGTH_LT, F_WIDTH_LT, 
CASE WHEN OUT_KNIFE_FILM.F_NEW='Y' THEN  QTY_KNIFE_FILM ELSE 0 END AS QTY_KNIFE_FILM
, (F_LENGTH*F_WIDTH*QTY_KNIFE_FILM)*1.0/100 AS OUT_SQM, FILM_M100_SQM.FILM_SQM,FILM_M100_SQM.PROD_PRINT_TIMES, OUT_KNIFE_FILM.F_NEW
FROM OUT_KNIFE_FILM 
LEFT JOIN ZTB_QLSXPLAN ON(ZTB_QLSXPLAN.PLAN_ID = OUT_KNIFE_FILM.PLAN_ID) 
LEFT JOIN FILM_M100_SQM ON (FILM_M100_SQM.G_CODE =ZTB_QLSXPLAN.G_CODE)
WHERE LOAIBANGIAO_PDP ='F' AND ZTB_QLSXPLAN.G_CODE is not null
