...
exports.checkMYCHAMCONG = async (req, res, DATA) => {
  const data = DATA || req.body; // Lấy DATA từ body
  const EMPL_NO = req.payload_data["EMPL_NO"]; // Lấy từ middleware checkLoginIndex
  const PASSWORD = req.payload_data["PASSWORD"];
  try {
    // Truy vấn thời gian check-in/check-out trong ngày hiện tại
    const setpdQuery = `
      SELECT MIN(C001.CHECK_DATETIME) AS MIN_TIME, MAX(C001.CHECK_DATETIME) AS MAX_TIME  
      FROM C001 
      LEFT JOIN ZTBEMPLINFO ON (C001.NV_CCID = ZTBEMPLINFO.NV_CCID AND C001.CTR_CD = ZTBEMPLINFO.CTR_CD) 
      WHERE C001.CHECK_DATE = CAST(GETDATE() as date) 
      AND ZTBEMPLINFO.EMPL_NO = '${EMPL_NO}' 
      AND C001.CTR_CD = '${data.CTR_CD}'
    `;
    const checkkq = await queryDB(setpdQuery);
    // Truy vấn thông tin nhân viên chi tiết
    const query = `
      SELECT ZTBEMPLINFO.EMPL_IMAGE, ZTBEMPLINFO.CTR_CD, ZTBEMPLINFO.EMPL_NO, ZTBEMPLINFO.CMS_ID, 
             ZTBEMPLINFO.FIRST_NAME, ZTBEMPLINFO.MIDLAST_NAME, ZTBEMPLINFO.DOB, ZTBEMPLINFO.HOMETOWN, 
             ZTBEMPLINFO.SEX_CODE, ZTBEMPLINFO.ADD_PROVINCE, ZTBEMPLINFO.ADD_DISTRICT, 
             ZTBEMPLINFO.ADD_COMMUNE, ZTBEMPLINFO.ADD_VILLAGE, ZTBEMPLINFO.PHONE_NUMBER, 
             ZTBEMPLINFO.WORK_START_DATE, ZTBEMPLINFO.PASSWORD, ZTBEMPLINFO.EMAIL, 
             ZTBEMPLINFO.WORK_POSITION_CODE, ZTBEMPLINFO.WORK_SHIFT_CODE, ZTBEMPLINFO.POSITION_CODE, 
             ZTBEMPLINFO.JOB_CODE, ZTBEMPLINFO.FACTORY_CODE, ZTBEMPLINFO.WORK_STATUS_CODE, 
             ZTBEMPLINFO.REMARK, ZTBEMPLINFO.ONLINE_DATETIME, ZTBSEX.SEX_NAME, ZTBSEX.SEX_NAME_KR, 
             ZTBWORKSTATUS.WORK_STATUS_NAME, ZTBWORKSTATUS.WORK_STATUS_NAME_KR, 
             ZTBFACTORY.FACTORY_NAME, ZTBFACTORY.FACTORY_NAME_KR, ZTBJOB.JOB_NAME, ZTBJOB.JOB_NAME_KR, 
             ZTBPOSITION.POSITION_NAME, ZTBPOSITION.POSITION_NAME_KR, ZTBWORKSHIFT.WORK_SHIF_NAME, 
             ZTBWORKSHIFT.WORK_SHIF_NAME_KR, ZTBWORKPOSITION.SUBDEPTCODE, ZTBWORKPOSITION.WORK_POSITION_NAME, 
      ...
      WHERE ZTBEMPLINFO.EMPL_NO = '${EMPL_NO}' 
      AND PASSWORD = '${PASSWORD}' 
      AND ZTBEMPLINFO.CTR_CD = '${data.CTR_CD}'
    `;
    const kqua = await queryDB(query);
    const loginResult = kqua.data; // Lấy dữ liệu từ result
    // Tạo token
    let token = "";
    if (loginResult.length > 0) {
      token = jwt.sign({ payload: JSON.stringify(loginResult) }, "nguyenvanhung", {
        expiresIn: 3600 * 24 * 1, // 24 giờ
      });
    } else if (loginResult.length === 0 && EMPL_NO === "NHU1903") {
      token = req.cookies.token; // Giữ token cũ nếu là NHU1903
    }
    // Chuẩn bị kết quả trả về
    let checkkq_token;
    if (EMPL_NO === "NHU1903") {
      checkkq_token = {
        ...checkkq, // Giả định checkkq.data là mảng, lấy phần tử đầu tiên
        REFRESH_TOKEN:
    ...
    console.error("Check MYCHAMCONG error:", error);
    res.send({ tk_status: "ng", message: "Error processing checkMYCHAMCONG" });
  }
};
