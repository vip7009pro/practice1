...
exports.process_api = function (req, res) {
 ...
 ZTB_QLSXPLAN.G_CODE, ZTB_QLSXPLAN.PLAN_ID,ZTB_QLSXPLAN.PLAN_DATE, ZTB_QLSXPLAN.PROD_REQUEST_NO, M100.G_NAME, M100.G_NAME_KD, ZTB_QLSXPLAN.PLAN_QTY, ZTB_QLSXPLAN.PLAN_EQ, ZTB_QLSXPLAN.PLAN_FACTORY, ZTB_QLSXPLAN.PROCESS_NUMBER, ZTB_QLSXPLAN.STEP, OUTVL.M_NAME, OUTVL.TOTAL_OUT_QTY,(OUTVL.TOTAL_OUT_QTY-isnull( TON_RETURN_TB.TOTAL_RETURN,0)) AS USED_QTY, (isnull(TON_RETURN_TB.TOTAL_RETURN,0)) AS REMAIN_QTY, ZTB_SX_RESULT.PD, ZTB_SX_RESULT.CAVITY, ZTB_SX_RESULT.SETTING_MET, CAST(((OUTVL.TOTAL_OUT_QTY-  TON_RETURN_TB.TOTAL_RETURN) - ZTB_SX_RESULT.SETTING_MET)/ZTB_SX_RESULT.PD * ZTB_SX_RESULT.CAVITY*1000 AS int) AS ESTIMATED_QTY,ZTB_QLSXPLAN.KETQUASX, INSPECT_INPUT_TABLE.INS_INPUT,  INSPECT_OUTPUT_TABLE.INS_OUTPUT,  ZTB_SX_RESULT.SETTING_START_TIME, ZTB_SX_RESULT.MASS_START_TIME, ZTB_SX_RESULT.MASS_END_TIME, ZTB_SX_RESULT.RPM,ZTB_SX_RESULT.EQ_NAME AS EQ_NAME_TT,ZTB_SX_RESULT.SX_DATE, ZTB_SX_RESULT.WORK_SHIFT, ZTB_SX_RESULT.INS_EMPL, ZTB_SX_EFFICIENCY.FACTORY, ZTB_SX_EFFICIENCY.BOC_KIEM, ZTB_SX_EFFICIENCY.LAY_DO, ZTB_SX_EFFICIENCY.MAY_HONG, ZTB_SX_EFFICIENCY.DAO_NG, ZTB_SX_EFFICIENCY.CHO_LIEU, ZTB_SX_EFFICIENCY.CHO_BTP, ZTB_SX_EFFICIENCY.HET_LIEU, ZTB_SX_EFFICIENCY.LIEU_NG, ZTB_SX_EFFICIENCY.CAN_HANG, ZTB_SX_EFFICIENCY.HOP_FL, ZTB_SX_EFFICIENCY.CHO_QC, ZTB_SX_EFFICIENCY.CHOT_BAOCAO, ZTB_SX_EFFICIENCY.CHUYEN_CODE, ZTB_SX_EFFICIENCY.KHAC, ZTB_SX_EFFICIENCY.REMARK
    FROM ZTB_QLSXPLAN
	LEFT JOIN (

	SELECT IN_KHO_SX.PLAN_ID_SUDUNG, M090.M_NAME,SUM(isnull(TOTAL_IN_QTY,0)) AS TOTAL_RETURN FROM IN_KHO_SX LEFT JOIN M090 ON (M090.M_CODE =IN_KHO_SX.M_CODE )  WHERE PHANLOAI='R' GROUP BY  IN_KHO_SX.PLAN_ID_SUDUNG,  M090.M_NAME

	) AS TON_RETURN_TB ON (TON_RETURN_TB.PLAN_ID_SUDUNG =ZTB_QLSXPLAN.PLAN_ID)
    LEFT JOIN
    (


	SELECT IN_KHO_SX.PLAN_ID_SUDUNG, M090.M_NAME,SUM(isnull(TOTAL_IN_QTY,0)) AS TOTAL_OUT_QTY FROM IN_KHO_SX LEFT JOIN M090 ON (M090.M_CODE =IN_KHO_SX.M_CODE )  WHERE PHANLOAI='N' GROUP BY  IN_KHO_SX.PLAN_ID_SUDUNG,  M090.M_NAME



    ) AS OUTVL ON (OUTVL.PLAN_ID_SUDUNG =ZTB_QLSXPLAN.PLAN_ID)
    ...
    else if (qr['command'] == 'loadDataSX_YCSX')
    {
        (async () => {
            ...
            let setpdQuery = `
            SELECT P400.G_CODE, CASE WHEN P400.CODE_55 = '04' THEN 'SAMPLE' ELSE 'MASS' END AS PHAN_LOAI, P400.PROD_REQUEST_NO, M100.G_NAME,M100.G_NAME_KD,M100.FACTORY, M100.EQ1, M100.EQ2, P400.PROD_REQUEST_DATE, P400.PROD_REQUEST_QTY, isnull(OUTKHO.M_NAME,'X') AS M_NAME, isnull(OUTKHO.TOTAL_OUTPUT,0) AS M_OUTPUT,(isnull(RETURN_TABLE.REMAIN_QTY,0)) AS REMAIN_QTY, (isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(RETURN_TABLE.REMAIN_QTY,0)) AS USED_QTY,  M100.PD, M100.G_C* M100.G_C_R AS CAVITY, CAST((isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(RETURN_TABLE.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000 AS int) AS ESTIMATED_QTY, isnull(KQSXTABLE.CD1,0) AS CD1, isnull(KQSXTABLE.CD2,0) AS CD2,  isnull(INS_INPUT_TB.INS_INPUT,0) INS_INPUT, isnull(INS_OUTPUT_TB.INS_OUTPUT,0)
            AS INS_OUTPUT,
            ...
            LEFT JOIN 
            ( 
				SELECT ZTB_QLSXPLAN.PROD_REQUEST_NO, M090.M_NAME,SUM(isnull(TOTAL_IN_QTY,0)) AS TOTAL_OUTPUT FROM IN_KHO_SX LEFT JOIN M090 ON (M090.M_CODE =IN_KHO_SX.M_CODE ) LEFT JOIN ZTB_QLSXPLAN ON (IN_KHO_SX.PLAN_ID_SUDUNG =ZTB_QLSXPLAN.PLAN_ID)  WHERE PHANLOAI='N' GROUP BY  ZTB_QLSXPLAN.PROD_REQUEST_NO,  M090.M_NAME
