    ...
    switch (qr["command"]) {
      ...
      case "updateemployee":
        (async () => {
          ...
          if (JOB_NAME === "Leader" || JOB_NAME === "ADMIN") {
            let checkkq = "OK";
            let checkresigndate = DATA.WORK_STATUS_CODE ===0 ? `, RESIGN_DATE='${DATA.RESIGN_DATE}'`: ``;
      ...
      case "mydiemdanhnhom":
        (async () => {
          ...
          let query =
           ...
           FROM 
             ZTBATTENDANCETB 
             LEFT JOIN ZTBEMPLINFO ON (
               ZTBEMPLINFO.EMPL_NO = ZTBATTENDANCETB.EMPL_NO
             ) 
             LEFT JOIN ZTBSEX ON (
           ...
           WHERE 
             ZTBATTENDANCETB.EMPL_NO = @empl
           ) AS AA
           ON (DATETABLE.DATE_COLUMN= AA.APPLY_DATE)
           WHERE DATETABLE.DATE_COLUMN BETWEEN @startdate
             AND @enddate
...
ON (DATEADD(day,+1,CHAMCONG0.CHECK_DATE) = ZTBEMPLINFOA.DATE_COLUMN AND CHAMCONG0.NV_CCID = ZTBEMPLINFOA.NV_CCID)
 WHERE ZTBEMPLINFOA.WORK_STATUS_CODE =1  AND ZTBEMPLINFOA.CMS_ID='CMS1549'
 ORDER BY ZTBEMPLINFOA.DATE_COLUMN DESC, ZTBEMPLINFOA.POSITION_CODE ASC
          `;
          //console.log(setpdQuery); 
          checkkq = await queryDB(setpdQuery);
