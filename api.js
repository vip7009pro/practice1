...
exports.process_api = function (req, res) {
    ...
    else if (qr['command'] == 'insert_invoice')
    {
        (async () => {            
            ...
            let currenttime = moment().format('YYYY-MM-DD HH:mm:ss'); 
            let checkkq = "OK";
            let setpdQuery = `INSERT INTO ZTBDelivery (CTR_CD, CUST_CD, EMPL_NO, G_CODE, PO_NO, DELIVERY_QTY, DELIVERY_DATE, NOCANCEL, REMARK, INVOICE_NO) VALUES ('002','${DATA.CUST_CD}', '${DATA.EMPL_NO}', '${DATA.G_CODE}', '${DATA.PO_NO}', '${DATA.DELIVERY_QTY}', '${DATA.DELIVERY_DATE}', 1,'${DATA.REMARK}', '${DATA.INVOICE_NO}')`;
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            console.log(checkkq);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'insert_p500')
    {
        (async () => {            
            ...
            console.log(DATA);            
            let checkkq = "OK";
            let setpdQuery = `INSERT INTO P500 (CTR_CD, PROCESS_IN_DATE, PROCESS_IN_NO, PROCESS_IN_SEQ, M_LOT_IN_SEQ, PROD_REQUEST_DATE, PROD_REQUEST_NO, G_CODE, M_CODE, M_LOT_NO, EMPL_NO, EQUIPMENT_CD, SCAN_RESULT, INS_DATE, INS_EMPL, UPD_DATE, UPD_EMPL, FACTORY) VALUES ('002','${DATA.in_date}','${DATA.next_process_in_no}','${DATA.PROD_REQUEST_NO.substring(4,7)}','${DATA.PROD_REQUEST_DATE.substring(5,8)}','${DATA.PROD_REQUEST_DATE}','${DATA.PROD_REQUEST_NO}','${DATA.G_CODE}', '','','${DATA.EMPL_NO}','${DATA.phanloai}01','OK',GETDATE(),'${DATA.EMPL_NO}',GETDATE(),'${DATA.EMPL_NO}','NM1')`;
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            console.log(checkkq);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'insert_p501')
    {
        (async () => {            
            ...
            let currenttime = moment().format('YYYY-MM-DD HH:mm:ss'); 
            let checkkq = "OK";
            let setpdQuery = `INSERT INTO P501 (CTR_CD,PROCESS_IN_DATE,PROCESS_IN_NO,PROCESS_IN_SEQ,M_LOT_IN_SEQ,PROCESS_PRT_SEQ,M_LOT_NO,PROCESS_LOT_NO,INS_DATE,INS_EMPL,UPD_DATE,UPD_EMPL) VALUES  ('002','${DATA.in_date}','${DATA.next_process_in_no}','${DATA.PROD_REQUEST_NO.substring(4,7)}','${DATA.PROD_REQUEST_DATE.substring(5,8)}','${DATA.next_process_prt_seq}','','${DATA.next_process_lot_no}',GETDATE(),'${DATA.EMPL_NO}',GETDATE(),'${DATA.EMPL_NO}')`;
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            console.log(checkkq);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'traxuatlieu')
    {
        (async () => {            
            ...
            FROM O302
            JOIN O301 ON (O302.OUT_DATE = O301.OUT_DATE AND O302.OUT_NO = O301.OUT_NO)
            JOIN O300 ON (O300.OUT_DATE = O301.OUT_DATE AND O300.OUT_NO = O301.OUT_NO)
            JOIN P400 ON (O300.PROD_REQUEST_NO = P400.PROD_REQUEST_NO) 
            JOIN M100 ON (P400.G_CODE = M100.G_CODE)
            JOIN M090 ON (M090.M_CODE = O302.M_CODE) 
            ${condition}
    ...
    else if (qr['command'] == 'insertM100_AddVer')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
                let checkkq = "OK";
                let setpdQuery = `INSERT INTO M100 (CTR_CD,CUST_CD,PROD_PROJECT,PROD_MODEL,CODE_12,PROD_TYPE,G_NAME_KD,DESCR,PROD_MAIN_MATERIAL,G_NAME,G_LENGTH,G_WIDTH,PD,G_C,G_C_R,G_SG_L,G_SG_R,PACK_DRT,KNIFE_TYPE,KNIFE_LIFECYCLE,KNIFE_PRICE,CODE_33,ROLE_EA_QTY,RPM,PIN_DISTANCE,PROCESS_TYPE,EQ1,EQ2,PROD_DIECUT_STEP,PROD_PRINT_TIMES,REMK,USE_YN,G_CODE,G_CG,G_LG, SEQ_NO, CODE_27, REV_NO, INS_EMPL, UPD_EMPL, INS_DATE, UPD_DATE) VALUES ('002','${DATA.CODE_FULL_INFO.CUST_CD}','${DATA.CODE_FULL_INFO.PROD_PROJECT}','${DATA.CODE_FULL_INFO.PROD_MODEL}','${DATA.CODE_FULL_INFO.CODE_12}','${DATA.CODE_FULL_INFO.PROD_TYPE}','${DATA.CODE_FULL_INFO.G_NAME_KD}','${DATA.CODE_FULL_INFO.DESCR}','${DATA.CODE_FULL_INFO.PROD_MAIN_MATERIAL}','${DATA.CODE_FULL_INFO.G_NAME}','${DATA.CODE_FULL_INFO.G_LENGTH}','${DATA.CODE_FULL_INFO.G_WIDTH}','${DATA.CODE_FULL_INFO.PD}','${DATA.CODE_FULL_INFO.G_C}','${DATA.CODE_FULL_INFO.G_C_R}','${DATA.CODE_FULL_INFO.G_SG_L}','${DATA.CODE_FULL_INFO.G_SG_R}','${DATA.CODE_FULL_INFO.PACK_DRT}','${DATA.CODE_FULL_INFO.KNIFE_TYPE}','${DATA.CODE_FULL_INFO.KNIFE_LIFECYCLE}','${DATA.CODE_FULL_INFO.KNIFE_PRICE}','${DATA.CODE_FULL_INFO.CODE_33}','${DATA.CODE_FULL_INFO.ROLE_EA_QTY}','${DATA.CODE_FULL_INFO.RPM}','${DATA.CODE_FULL_INFO.PIN_DISTANCE}','${DATA.CODE_FULL_INFO.PROCESS_TYPE}','${DATA.CODE_FULL_INFO.EQ1}','${DATA.CODE_FULL_INFO.EQ2}','${DATA.CODE_FULL_INFO.PROD_DIECUT_STEP}','${DATA.CODE_FULL_INFO.PROD_PRINT_TIMES}','${DATA.CODE_FULL_INFO.REMK}','${DATA.CODE_FULL_INFO.USE_YN}','${DATA.G_CODE}','${DATA.CODE_FULL_INFO.G_CG}','${DATA.CODE_FULL_INFO.G_LG}','${DATA.NEXT_SEQ_NO}','${DATA.CODE_27}','${DATA.REV_NO}','${EMPL_NO}','${EMPL_NO}',GETDATE(), GETDATE())`;
                console.log(setpdQuery);       
                checkkq = await queryDB(setpdQuery);
                res.send(checkkq); 
                //console.log(checkkq);                   
    ...
    else if (qr['command'] == 'updateLIEUQL_SX_M140')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
            let checkkq = "OK";
            let setpdQuery = `UPDATE M140 SET LIEUQL_SX=${DATA.LIEUQL_SX} WHERE G_CODE='${DATA.G_CODE}' AND M_CODE='${DATA.M_CODE}'`; 
            //${moment().format('YYYY-MM-DD')}
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'checktonlieutrongxuong')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
            let checkkq = "OK";
            let setpdQuery = `SELECT IN_KHO_SX.FACTORY, IN_KHO_SX.PHANLOAI, IN_KHO_SX.PLAN_ID_INPUT, IN_KHO_SX.M_CODE, M090.M_NAME, M090.WIDTH_CD, IN_KHO_SX.M_LOT_NO, IN_KHO_SX.ROLL_QTY, IN_KHO_SX.IN_QTY, IN_KHO_SX.TOTAL_IN_QTY  FROM IN_KHO_SX LEFT JOIN M090 ON  (M090.M_CODE= IN_KHO_SX.M_CODE) WHERE IN_KHO_SX.USE_YN='Y'`; 
            //${moment().format('YYYY-MM-DD')}
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'xuatkhoao')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
            let checkkq = "OK";
            let setpdQuery = `INSERT INTO OUT_KHO_SX (CTR_CD,FACTORY,PHANLOAI,PLAN_ID_INPUT,PLAN_ID_OUTPUT,M_CODE,M_LOT_NO,ROLL_QTY,OUT_QTY,TOTAL_OUT_QTY,USE_YN,INS_DATE,INS_EMPL,UPD_DATE,UPD_EMPL) VALUES ('002','${DATA.FACTORY}','${DATA.PHANLOAI}', '${DATA.PLAN_ID_INPUT}', '${DATA.PLAN_ID_OUTPUT}','${DATA.M_CODE}','${DATA.M_LOT_NO}','${DATA.ROLL_QTY}','${DATA.OUT_QTY}','${DATA.TOTAL_OUT_QTY}','${DATA.USE_YN}', GETDATE(), '${EMPL_NO}', GETDATE(),'${EMPL_NO}')`; 
            //${moment().format('YYYY-MM-DD')}
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'lichsuxuatkhoao')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
            let checkkq = "OK";
            let setpdQuery = `SELECT OUT_KHO_SX.FACTORY, OUT_KHO_SX.PHANLOAI, OUT_KHO_SX.M_CODE, M090.M_NAME, M090.WIDTH_CD, OUT_KHO_SX.M_LOT_NO, OUT_KHO_SX.PLAN_ID_INPUT,OUT_KHO_SX.PLAN_ID_OUTPUT, OUT_KHO_SX.ROLL_QTY, OUT_KHO_SX.OUT_QTY, OUT_KHO_SX.TOTAL_OUT_QTY, OUT_KHO_SX.INS_DATE FROM OUT_KHO_SX LEFT JOIN M090 ON (M090.M_CODE = OUT_KHO_SX.M_CODE) ORDER BY OUT_KHO_SX.INS_DATE DESC`; 
            //${moment().format('YYYY-MM-DD')}
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'lichsuinputlieusanxuat_full')
    {
        (async () => {
            ...
            LEFT JOIN M090 ON (M090.M_CODE = P500.M_CODE)
            LEFT JOIN  
            (SELECT PLAN_ID_OUTPUT, M_CODE, M_LOT_NO, isnull(SUM(TOTAL_OUT_QTY),0) AS TOTAL_OUT_QTY  FROM OUT_KHO_SX  GROUP BY PLAN_ID_OUTPUT, M_CODE,M_LOT_NO) AS BB ON (BB.PLAN_ID_OUTPUT = P500.PLAN_ID AND BB.M_CODE = P500.M_CODE AND BB.M_LOT_NO=P500.M_LOT_NO) ${condition}
    ...
    else if (qr['command'] == 'setUSE_YN_KHO_AO_INPUT')
    {
        (async () => {
            ...
            let SUBDEPTNAME = req.payload_data['SUBDEPTNAME']; 
            let checkkq = "OK";
            let setpdQuery = `UPDATE IN_KHO_SX SET USE_YN ='${DATA.USE_YN}', PLAN_ID_SUDUNG='${DATA.PLAN_ID_SUDUNG}' WHERE PLAN_ID_INPUT= '${DATA.PLAN_ID_INPUT}' AND M_CODE='${DATA.M_CODE}' AND M_LOT_NO='${DATA.M_LOT_NO}' AND TOTAL_IN_QTY=${DATA.TOTAL_IN_QTY}`; 
            //${moment().format('YYYY-MM-DD')}
            console.log(setpdQuery);       
            checkkq = await queryDB(setpdQuery);
            res.send(checkkq);             
    ...
    else if (qr['command'] == 'loadDataSX')
    {
        (async () => {
            ...
            let setpdQuery = `
            SELECT ZTB_QLSXPLAN.PLAN_ID,ZTB_QLSXPLAN.PLAN_DATE, ZTB_QLSXPLAN.PROD_REQUEST_NO, M100.G_NAME, M100.G_NAME_KD, ZTB_QLSXPLAN.PLAN_QTY, ZTB_QLSXPLAN.PLAN_EQ, ZTB_QLSXPLAN.PLAN_FACTORY, ZTB_QLSXPLAN.PROCESS_NUMBER, ZTB_QLSXPLAN.STEP, OUTVL.M_NAME, OUTVL.TOTAL_OUT_QTY,(OUTVL.TOTAL_OUT_QTY- P500_A.REMAIN_QTY) AS USED_QTY, P500_A.REMAIN_QTY, ZTB_SX_RESULT.PD, ZTB_SX_RESULT.CAVITY, ZTB_SX_RESULT.SETTING_MET, CAST(((OUTVL.TOTAL_OUT_QTY- P500_A.REMAIN_QTY) - ZTB_SX_RESULT.SETTING_MET)/ZTB_SX_RESULT.PD * ZTB_SX_RESULT.CAVITY*1000 AS int) AS ESTIMATED_QTY,ZTB_QLSXPLAN.KETQUASX, INSPECT_INPUT_TABLE.INS_INPUT,  INSPECT_OUTPUT_TABLE.INS_OUTPUT,  ZTB_SX_RESULT.SETTING_START_TIME, ZTB_SX_RESULT.MASS_START_TIME, ZTB_SX_RESULT.MASS_END_TIME, ZTB_SX_RESULT.RPM,ZTB_SX_RESULT.EQ_NAME AS EQ_NAME_TT,ZTB_SX_RESULT.SX_DATE, ZTB_SX_RESULT.WORK_SHIFT, ZTB_SX_RESULT.INS_EMPL, ZTB_SX_EFFICIENCY.FACTORY, ZTB_SX_EFFICIENCY.BOC_KIEM, ZTB_SX_EFFICIENCY.LAY_DO, ZTB_SX_EFFICIENCY.MAY_HONG, ZTB_SX_EFFICIENCY.DAO_NG, ZTB_SX_EFFICIENCY.CHO_LIEU, ZTB_SX_EFFICIENCY.CHO_BTP, ZTB_SX_EFFICIENCY.HET_LIEU, ZTB_SX_EFFICIENCY.LIEU_NG, ZTB_SX_EFFICIENCY.CAN_HANG, ZTB_SX_EFFICIENCY.HOP_FL, ZTB_SX_EFFICIENCY.CHO_QC, ZTB_SX_EFFICIENCY.CHOT_BAOCAO, ZTB_SX_EFFICIENCY.CHUYEN_CODE, ZTB_SX_EFFICIENCY.KHAC, ZTB_SX_EFFICIENCY.REMARK
                        FROM ZTB_QLSXPLAN
                        LEFT JOIN
                        (
                        SELECT ZTB_QLSXPLAN.PLAN_ID, M090.M_NAME, SUM(isnull(OUT_KHO_SX_AA.TOTAL_OUT_QTY,0)) AS TOTAL_OUT_QTY  FROM ZTB_QLSXPLAN
                        LEFT JOIN ZTB_QLSXCHITHI ON(ZTB_QLSXPLAN.PLAN_ID = ZTB_QLSXCHITHI.PLAN_ID)
                        LEFT JOIN
                        (SELECT PLAN_ID_OUTPUT, M_CODE, SUM(TOTAL_OUT_QTY) AS TOTAL_OUT_QTY FROM OUT_KHO_SX WHERE OUT_KHO_SX.PHANLOAI='N' GROUP BY PLAN_ID_OUTPUT, M_CODE) AS OUT_KHO_SX_AA
                        ON (OUT_KHO_SX_AA.PLAN_ID_OUTPUT= ZTB_QLSXCHITHI.PLAN_ID AND OUT_KHO_SX_AA.M_CODE= ZTB_QLSXCHITHI.M_CODE)
                        LEFT JOIN M090 ON (M090.M_CODE = ZTB_QLSXCHITHI.M_CODE)
                        WHERE ZTB_QLSXCHITHI.LIEUQL_SX =1
                        GROUP BY ZTB_QLSXPLAN.PLAN_ID, M090.M_NAME
                        ) AS OUTVL ON (OUTVL.PLAN_ID =ZTB_QLSXPLAN.PLAN_ID)
                        LEFT JOIN (SELECT P500.PLAN_ID, M090.M_NAME, SUM(P500.REMAIN_QTY) AS REMAIN_QTY FROM P500 JOIN M090 ON (M090.M_CODE=  P500.M_CODE) GROUP BY P500.PLAN_ID,M090.M_NAME) AS P500_A ON (P500_A.PLAN_ID = ZTB_QLSXPLAN.PLAN_ID)
    ...
    else if (qr['command'] == 'loadDataSX_YCSX')
    {
        (async () => {
            ...
            if(DATA.M_NAME !=='')
            {
                condition+= ` AND OUTKHO.M_NAME LIKE '%${DATA.M_NAME}%'`
            }            
            
            let setpdQuery = `
            SELECT P400.PROD_REQUEST_NO, M100.G_NAME,M100.G_NAME_KD,M100.FACTORY, M100.EQ1, M100.EQ2, P400.PROD_REQUEST_DATE, P400.PROD_REQUEST_QTY, isnull(OUTKHO.M_NAME,'X') AS M_NAME, isnull(OUTKHO.TOTAL_OUTPUT,0) AS M_OUTPUT,isnull(P500_A.REMAIN_QTY,0) AS REMAIN_QTY, (isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) AS USED_QTY,  M100.PD, M100.G_C* M100.G_C_R AS CAVITY, CAST((isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000 AS int) AS ESTIMATED_QTY, isnull(KQSXTABLE.CD1,0) AS CD1, isnull(KQSXTABLE.CD2,0) AS CD2,  isnull(INS_INPUT_TB.INS_INPUT,0) INS_INPUT, isnull(INS_OUTPUT_TB.INS_OUTPUT,0)
            AS INS_OUTPUT,
            CASE WHEN CAST((isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000 AS int) <>0 THEN 1-isnull(KQSXTABLE.CD1,0) /CAST((isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000 AS float) ELSE 0 END AS LOSS_SX1,
            CASE 
            ...
            END AS LOSS_SX4,
            CASE 
            WHEN (isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000  <>0 THEN 1-CAST(isnull(INS_OUTPUT_TB.INS_OUTPUT,0) as float) /Cast((isnull(OUTKHO.TOTAL_OUTPUT,0)- isnull(P500_A.REMAIN_QTY,0)) / M100.PD * (M100.G_C* M100.G_C_R)*1000 as float)
            ELSE 0
            END AS TOTAL_LOSS
            ...
            LEFT JOIN 
            ( 
                SELECT SUBSTRING(PLAN_ID_OUTPUT,0,8)AS PROD_REQUEST_NO , M090.M_NAME, SUM(isnull(OUT_KHO_SX.TOTAL_OUT_QTY,0)) AS TOTAL_OUTPUT  FROM OUT_KHO_SX JOIN M090 ON M090.M_CODE= OUT_KHO_SX.M_CODE WHERE PHANLOAI = 'N' GROUP BY  SUBSTRING(PLAN_ID_OUTPUT,0,8), M090.M_NAME
            ) AS OUTKHO ON (P400.PROD_REQUEST_NO= OUTKHO.PROD_REQUEST_NO)
            LEFT JOIN 
            ...
            LEFT JOIN (SELECT P500.PROD_REQUEST_NO, M090.M_NAME, isnull(SUM(P500.REMAIN_QTY),0) AS REMAIN_QTY FROM P500 LEFT JOIN M090 ON (M090.M_CODE = P500.M_CODE) WHERE P500.PLAN_ID is not null GROUP BY P500.PROD_REQUEST_NO, M090.M_NAME) AS P500_A ON (P500_A.PROD_REQUEST_NO = P400.PROD_REQUEST_NO)
            ${condition}
    ...
    else if (qr['command'] == 'checkQLSXPLANSTATUS')
    {
        (async () => {
            ...
            let checkkq = "OK";
        let setpdQuery = `SELECT   ZTB_QLSXPLAN.PLAN_QTY, ZTB_QLSXPLAN.KQ_SX_TAM, ZTB_QLSXPLAN.KETQUASX, ZTB_QLSXPLAN.STEP, ZTB_QLSXPLAN.PLAN_FACTORY, ZTB_QLSXPLAN.PLAN_ID, ZTB_QLSXPLAN.PLAN_DATE,  ZTB_QLSXPLAN.PLAN_EQ, M100.G_NAME,M100.G_NAME_KD, OUT_KNIFE_FILM_A.PLAN_ID AS XUATDAO, ZTB_SX_RESULT.SETTING_START_TIME, ZTB_SX_RESULT.MASS_START_TIME, ZTB_SX_RESULT.MASS_END_TIME,O301_A.PLAN_ID AS DKXL, OUT_KHO_SX_A.PLAN_ID_OUTPUT AS XUATLIEU, ZTB_SX_RESULT.SX_RESULT AS CHOTBC
        FROM ZTB_QLSXPLAN
