...
interface TrainingExample {
  id: string;
  source_type: 'pattern' | 'relationship' | 'concept' | 'rule';
...
interface QueryPattern {
  ...
  optional_joins: Array<{ table: string; on: string }>;
...
export class TrainingService {
  ...
  constructor(metadataService: MetadataService) {
    ...
    this.trainingFile = path.join(__dirname, '../metadata/training.json');
  }
   ...
   */
  async learFromCommonPatterns(): Promise<QueryPattern[]> {
    try {
   ...
   */
  async generateVariationsFromRelationships(
    ...
    try {
      ...
      for (const table of allTables) {
        if (table.business_name.includes(businessConcept)) {
          ...
          variations.push(
            `Show ${businessConcept} grouped by ${table.table_name}`
          );
          ...
          for (const col of columns) {
            if (col.is_measure) {
              variations.push(
                `Total ${col.business_name} by ${table.business_name}`
              );
              variations.push(
                `Average ${col.business_name} per ${table.business_name}`
              );
            }
          }
        }
  ...
  async mapBusinessConceptToSQL(concept: {
    ...
    primary_metric: string; // e.g., "Revenue", "Count"
    dimension: string; // e.g., "Product", "Customer"
    time_period?: string; // e.g., "Monthly", "Quarterly"
    ...
    try {
      ...
      if (concept.time_period) {
        variations.push(
          `${concept.primary_metric} by ${concept.dimension} ${concept.time_period.toLowerCase()}`
        );
        variations.push(
          `${concept.time_period} ${concept.primary_metric.toLowerCase()} analysis by ${concept.dimension.toLowerCase()}`
        );
      }
  ...
  private addTrainingExample(example: TrainingExample): void {
    try {
      let training: { examples: TrainingExample[] } = { examples: [] };

      if (fs.existsSync(this.trainingFile)) {
        const content = fs.readFileSync(this.trainingFile, 'utf-8');
        training = JSON.parse(content);
      }

      training.examples.push(example);
      fs.writeFileSync(this.trainingFile, JSON.stringify(training, null, 2));
  ...
  deleteTrainingExample(id: string): void {
    try {
      ...
      training.examples = (training.examples || []).filter(
        (ex: TrainingExample) => ex.id !== id
      );
      fs.writeFileSync(this.trainingFile, JSON.stringify(training, null, 2));
...
export function createTrainingService(metadataService: MetadataService): TrainingService {
