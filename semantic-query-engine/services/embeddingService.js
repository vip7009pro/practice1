...
const logger = logger_1.createLogger('EmbeddingService');
class EmbeddingService {
    constructor(cacheDir = './semantic-query-engine/.embeddings') {
        this.embeddingModel = null;
        this.cache = {};
        this.initialized = false;
        this.cacheUpdateCounter = 0;
        this.CACHE_SAVE_INTERVAL = 10; // Save after every 10 new embeddings
        this.embeddingDimension = 256;
    ...
    async initialize() {
        ...
        try {
            ...
            this.embeddingModel = await transformers.pipeline('feature-extraction', 'Xenova/multilingual-e5-small');
            this.initialized = true;
    ...
    async embedWithXenova(text) {
        ...
        if (embedding && typeof embedding.data !== 'undefined') {
            ...
            if (typeof embedding.tolist === 'function') {
            ...
            else if (typeof embedding.toArray === 'function') {
                ...
                vector = Object.values(embedding).flat();
            }
        }
        else {
    ...
    embedLocally(text) {
        const normalizedText = helpers_1.normalizeText(text);
        const tokens = helpers_1.tokenize(normalizedText);
        ...
        for (const feature of features) {
            const digest = crypto_1.default.createHash('sha256').update(feature).digest();
        ...
        for (let i = 0; i + 2 < compactText.length; i++) {
            ...
            const digest = crypto_1.default.createHash('sha256').update(`gram:${trigram}`).digest();
        ...
        for (let i = 0; i + 1 < tokens.length; i++) {
            ...
            const digest = crypto_1.default.createHash('sha256').update(tokenPair).digest();
            const index = digest.readUInt32LE(0) % this.embeddingDimension;
            vector[index] += 0.75;
        }
     ...
     */
    async findSimilar(queryText, candidates, threshold = 0.5, topK = 10) {
        try {
