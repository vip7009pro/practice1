...
const logger = logger_1.createLogger('Formatter');
class Formatter {
     ...
     */
    async format(executionResult, originalQuery) {
        ...
        try {
            ...
            throw new types_1.SemanticEngineError('FORMATTING_FAILED', 'Result formatting failed', error);
        }
    }
    /**
     * Generate human-readable explanation
     */
    async generateExplanation(query, rows) {
        if (rows.length === 0) {
        ...
        if (numericColumns.length > 0) {
            ...
            for (const col of numericColumns.slice(0, 3)) {
                ...
                if (values.length > 0) {
                    ...
                    insights.push(`${col}: Cao nhất ${helpers_1.formatNumber(max)}, thấp nhất ${helpers_1.formatNumber(min)}, trung bình ${helpers_1.formatNumber(avg.toFixed(0))}`);
                }
            }
        }
        // Detect date columns
     ...
     */
    suggestVisualization(rows, columns) {
        if (rows.length === 0) {
            return undefined;
        }
        // Detect data types
        ...
        const dateCols = columns.filter((c) => firstRow[c] instanceof Date ||
        ...
        for (const col of columns) {
            ...
            if (value === null || value === undefined) {
                ...
                if (col.toLowerCase().includes('amount') || col.toLowerCase().includes('price')) {
                    formatted[col] = helpers_1.formatCurrency(value);
                }
                else {
                    formatted[col] = helpers_1.formatNumber(value);
                }
            }
